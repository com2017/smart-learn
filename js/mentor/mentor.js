                ...
                for(var key in courses['courses']) {
                    if(containsMentor(username, courses['courses'][key]['courseMentors'])) {
                        if($("#courses:contains('"+courses['courses'][key].courseTitle+"')").length == 0) {
                            ...
                            course += '<a href="course.html?course_id=' +
                            id + '">' +
...
function coursePending(attendants, course_id) {
    for(var key in attendants) {
        ...
        var userUrl = getHostRoot() + '/'+DHISFolder+ '/api/systemSettings/VJFS_' + attendee + '_questions';
        $.ajax({
            url: userUrl,
            dataType: 'json'
        }).success(function(q) {
...
function studentPending(student_username, course_id) {
    var userUrl = getHostRoot() + '/'+DHISFolder+'/api/systemSettings/VJFS_' + student_username + '_questions';
    $.ajax({
        url: userUrl,
        dataType: 'json'
    }).success(function(q) {
...
function displayQuizes(course_id, student_id) {
    // Get courses as json object
    getQuizes(course_id, student_id, function(v) { 
        for(key in v) {
            // Display quizes
            var quiz_id = v[key].quizID;
            if($("#quizes:contains('"+v[key].quizTitle+"')").length == 0) {
                var quiz = '<li id='+quiz_id+' class="list-group-item clearfix">';
                quiz += '<a href="students.html?student_id=' + student_id + '&quiz_id=' + quiz_id + '&quiz_id=' + course_id +'">'+ v[key].quizTitle + '</a>';
                quiz += '</li>';
                $('#quizes').append(quiz);
            }
        ...
        $("#quizes li").each(function( title ) {
            var checker = 0;
            for(var key2 in v) {
                if(($(this).text().indexOf(v[key2].quizTitle)) < 0) {
                    checker++;
                }
            }
            if(checker == v.length) {
...
function getQuizes(course_id, student_id, handler) {
    ...
    var studentQuizes = getHostRoot() + '/'+DHISFolder+'/api/dataStore/coursetaker/VJFS_' + student_id + '_quizes';
    var allQuizes = getHostRoot() + '/'+DHISFolder+'/api/systemSettings/VJFS_quizes';
    var v = [];
    ...
    $.ajax({
        ...
        $.ajax({
            ...
            for (var i = 0,max =quize.quizes.length; i < max; i++) {
            //console.log(quize.quizes[i].quizID);
            var ID = quize.quizes[i].quizID;
            //console.log(userQuizes['quizes']);
             
                if(containsQuiz(ID, course_id, userQuizes['quizes'])) {
                    v.push(userQuizes['quizes'][i]);
                }
                handler(v);
        }
        ...
        if(!quizQuestions || !quizQuestions.count) {
            for(q in quizQuestions) {
                ...
                for(userNum in userQuestions) {
                    if(userQuestions[userNum].quizID == quizQuestions[q].quizID) {
                        q2 = userNum;
                        break;
                    }
                }
                ...
                if( quizQuestions[q].questionType == "text"){
                    ...
                    for(var i = 0; i < num_alternatives; i++) {
                        var checked = quizQuestions[q2]['questionAlternatives'][i]['alternativeChecked'] ? "checked=" : "";
...
function getQuestions(quiz_id, student_username, handler) {
    // Get URL from where to fetch json
    var urlStudent = getHostRoot() + '/'+DHISFolder+'/api/dataStore/coursetaker/VJFS_' + student_username + '_quizes'; 
    var urlQuiz = getHostRoot() + '/'+DHISFolder+'/api/systemSettings/VJFS_questions';
    var uq = [];
    var qq = [];
    ...
    var a = $.ajax({
        url: urlStudent,
        dataType: 'json'
    }).success(function(userquestions) {
        for(key in userquestions['quizes']) {
            if(userquestions['quizes'][key].quizID == quiz_id) {
                uq.push(userquestions['quizes'][key]);
            }
        }
    }).error(function(error) {
        handler(null, null);
...
function saveCorrection(quiz_id, student_username, course_id) {
    ...
    var questionUrl = getHostRoot() + '/'+DHISFolder+'/api/systemSettings/VJFS_questions';
    ...
    if(allCorrect == 1) {
        $.ajax({
            ...
            var completeUrl = getHostRoot() + '/'+DHISFolder+'/api/dataStore/coursetaker/VJFS_' + student_username + '_quizes';
            $.ajax({
                url: completeUrl,
                dataType: 'json'
...
function containsQuiz(value, course, array) {
    ...
    for (var i = 0, max = array.length; i < max; i++) {     
        if(array[i].quizID == value && array[i].courseID == course) {
            return true;
        }
    }
    return false;
...
function postData(data, url) {
    $.ajax({
        ...
        window.location.href = getHostRoot() + '/'+getDHISInstallFolder()+'/api/apps/smart-learn/pages/mentor/mentor.html'
    }).error(function(error) {
        console.log(error);
    });
}
