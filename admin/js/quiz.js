...
function getQuizResourceURL() {
    var url = getApiRoot() + '/api/systemSettings/VJFS_quizes';
    return url;
}
...
function displayQuizes(course) {
    ...
    $('#courseQuizzes').append(' <label class="list-group-item active">Quiz</label>');
    $('#courseQuizzes').append('<ul class="list-group-item list-group" id="courseQuiz"></ul>');
    $('#courseQuizzes').append('<a href="quiz.html?course_id=' + course.courseID + '" class="btn btn-info">New Quiz</a>');
...
function getImageResourceURL(quiz_id){
    //var quiz_id = getURLParameter(window.location, 'quiz_id');
    var url = getApiRoot() + '/api/systemSettings/VJFS_Images_' + quiz_id;
    //console.log(url);
    return url;
}
