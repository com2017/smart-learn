...
function getCourseResourceURL() {
    var url = getApiRoot() + '/api/systemSettings/VJFS_courses';
    return url;
}
          ...
          if (users!=null) {
              ...
              for(key in users['users']) {
                if (users['users'][key].hasOwnProperty('userCredentials')) {
                  userRow += '<tr>';
                  username = users['users'][key].userCredentials.username;
...
function doLabels(username) {
    ...
    var url = getApiRoot() + '/api/dataStore/coursetaker/VJFS_'+username+'_quizes';
