...
function displayAttendants(course) {
    ...
    getUsers(function(users){
        ...
        setupBloodHound('bloodhound_user', 'users', users['users'], function(user) {
            ...
            return user['name'] + ' (' + user['userCredentials']['username'] + ')';
        });
        ...
        if(course['courseMentors']) {
            for(var i = 0; i < course['courseMentors'].length; i++) {
                for(var j = 0; j < users['users'].length; j++) {
                    ...
                    if(users['users'][j].hasOwnProperty('userCredentials')
                        && (course['courseMentors'][i]['mentorUsername'] == users['users'][j]['userCredentials']['username'])) {
...
function getUsers(handler) {
    ...
    var url = getApiRoot() + '/api/users.json?fields=name,id,userCredentials[username]&paging=false';
...
function getUserByUsername(username, handler) {
    ...
    var url = getApiRoot() + '/api/users.json?fields=name,id,userCredentials[username]&filter=userCredentials.username:like:' + username+'&paging=false';
...
function getUserByID(ID, handler) {
    // Get URL from where to fetch users's json
    // (This URL will only retrieve id, name and username)
    var url = getApiRoot() + '/api/users.json?fields=name,id,userCredentials[username]&filter=id' + ID + 'paging=false';
...
function deleteAttendant(course_id, attendantUsername) {
    ...
    var url = getApiRoot() + '/api/systemSettings/courses';
